...
const DRDetails = () => {
  ...
  useEffect(() => {
    axiosSecure.get(`donationdetails/${id}`).then((response) => {
  ...
  const handleDonateBtn = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axiosSecure.patch(`changetoinprogress/${data._id}`).then((res) => {
  ...
  return (
    <div className="grid place-items-center mx-auto p-11 mt-20 card  outline">
      ...
      {userData?.role === "volunteer" && (
        <Link to="/dashboard/volunteer-all-blood-donation-request">
          <button className="btn btn-primary mt-4 capitalize"> go back</button>
        </Link>
      )}
      {userData?.role === "admin" && (
